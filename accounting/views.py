...
class PartyLedgerJournalView(CreateView):
    ...
    def post(self, request, ledger_id):
        ...
        if credit_ledger_type in ['Asset', 'Expense']:
            ...
            credit_ledger.save()
            # update_cumulative_ledger_bill(credit_ledger, entry_date)
            update_cumulative_ledger_partyledger(credit_ledger, entry_date, journal_entry)
...
class PartyLedgerJournal1View(CreateView):
    ...
    def post(self, request, ledger_id):
        ...
        new_receipt = Receipt.objects.create(
            fiscal_year=Organization.objects.last().current_fiscal_year,  # or get dynamically
            agent_name=request.user.username,
            transaction_date_time=entry_date,
            transaction_date=entry_date.date(),
            sub_total=debit_amount,
            grand_total=debit_amount,
            receipt_number=None,  # or auto-generate
            payment_mode=None,  # or get from POST if available
            received_from_ledger=credit_ledger,
            received_to_ledger=debit_ledger,
        )
        # return redirect(current_page_url)
        # Generate URL to receipt detail page
        receipt_url = reverse('receipt_card', args=[new_receipt.id])
...
class LedgerDetailView(View):
    ...
    def get(self, request, ledger_id):
        ...
        context = {
            ...
            'opening_balance': {'credit': opening_balance_credit if opening_balance_credit else 0, 'debit': opening_balance_debit if opening_balance_debit else 0},
            'from_date': from_date,
            'to_date': to_date,
